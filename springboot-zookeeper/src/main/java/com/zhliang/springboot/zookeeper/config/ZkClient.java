    ...
    public void init() {
        try{
            RetryPolicy retryPolicy = new ExponentialBackoffRetry(zkProperties.getBaseSleepTimeMs(),
                    zkProperties.getMaxRetries());
            CuratorFrameworkFactory.Builder builder   = CuratorFrameworkFactory.builder()
                    .connectString(zkProperties.getServer()).retryPolicy(retryPolicy)
            ...
            if(StringUtils.isNotEmpty( zkProperties.getDigest())){
                builder.authorization("digest", zkProperties.getDigest().getBytes("UTF-8"));
